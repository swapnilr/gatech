...
void* gtcache_get(char *key, size_t *val_size){
  ...
  if(savedVal && strcmp(savedVal->key, key) == 0){
    ...
    *val_size = savedVal->val_size;
    ...
    indexminpq_changekey(priorityqueue, savedVal->id, (indexminpq_key) &(savedVal->lastTime));
    return retVal;
  }
  return (void *) NULL;
}
...
void addEntry(char *key, char *value, size_t val_size) {
  ...
  indexminpq_insert(priorityqueue, index, (indexminpq_key) &(entry->lastTime));
...
void evict(size_t val_size) {
  ...
  //printf("Evicting %s at index %d\n", evictee->key, evictee->id);
