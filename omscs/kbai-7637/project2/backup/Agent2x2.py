from RavensTransformationP2 import RavensTransformation
import random
    ...
    def Solve(self, problem):
        ...
        for ftf in self._transformations_["A-B"]:
            ...
            for candidateRT in candidateRTs:
               #print "%s" % candidateRT.goal.getName()
               otfOptions = {}
               for otf in ftf:
                   #options = candidateRT.getAllOTFs()
               ...
               valid = True
               numOptions = 0
               for otf, options in otfOptions.iteritems():
                   #print otf
                   #print options
                   #for o in options:
                   #    print o
                   ...
                   if len(options) == 0:
                       ...
                       break
               #print numOptions
               #if numOptions < len(C.getObjects()):
               #    valid = False
               if valid:
                    answer = candidateRT.goal.getName()
                    correct = problem.checkAnswer(answer)
                    if answer != correct:
                        print "Question %s Answer:%s Correct:%s" % (problem.getName(), answer, correct)
                    return answer
        # At this point, try again with matching objects WITH deletion + multimap
        print "Question %s: Guessing!" % problem.getName()
