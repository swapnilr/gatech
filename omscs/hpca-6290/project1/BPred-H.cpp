...
BPHybrid::BPHybrid(int32_t i, int32_t fetchWidth, const char *section)
    ...
    ,sizeMask(SescConf->getInt(section,"localSize")-1)

{
    ...
    //std::map<uint32_t, metrics_t> metrics;
    tagArray = new uchar[SescConf->getInt(section,"localSize")];
}

BPHybrid::~BPHybrid()
{
  FILE *fp;
  fp = fopen("BranchStats.csv", "a+");
  std::vector<std::pair<long, metrics_t>> items;
  long total = 0;
  for(auto& metric: metrics) {
    metrics_t values = metric.second;
    items.push_back(std::make_pair(values.instructions, values));
    total += values.instructions;
  }
  auto cmp = [](std::pair<long, metrics_t> const &a, std::pair<long, metrics_t> const &b) {
    return a.first >  b.first;
  };
  double to_keep = (0.99) * total;
  long to_be_kept = 0;
  std::sort(items.begin(), items.end(), cmp);
  for(auto& p: items) {
    metrics_t values = p.second;
    to_be_kept += values.instructions;
    if(to_be_kept > to_keep) {
      break;
    } else {
      int val = fprintf(fp, "%08X,%010ld,%.3f,%.3f,%.3f\n", values.addr,
                                               values.instructions,
                                               double(values.overall)/values.instructions,
                                               double(values.global)/values.instructions,
                                               double(values.local)/values.instructions);
    }
  }
  fclose(fp);
  delete []tagArray;
...
PredType BPHybrid::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
    ...
    // calculate Table possition
    l2Index = ((l2Index ^ iID) & historyMask ) | (iID<<historySize);
    uchar topTwoBytes = (inst->getAddr() & 65280) >> 8;//16711680) >> 16;
    //printf("%X, %X\n", inst->getAddr(), topTwoBytes);
    bool globalTaken;
    bool localTaken;
    bool ptaken;
    // Check local. If in local, predict based on that
    //              also predict global
    // Otherwise send result of global(+ predict)
    // If global result is not equal to taken, predict local
    if(topTwoBytes == tagArray[iID & sizeMask]) {
      // Current instruction is answered by local
      if(doUpdate) {
      ...
      if(globalTaken != taken) {
          tagArray[iID & sizeMask] = topTwoBytes;
      }
    }

    /*if (doUpdate) {
        globalTaken = globalTable.predict(l2Index, taken);
        tagArray[l2Index & sizeMask] = topTwoBytes;
        localTaken  = localTable.predict(iID, taken);
    } else {
        globalTaken = globalTable.predict(l2Index);
        localTaken  = localTable.predict(iID);
    ...
    metrics_t this_metrics = metrics[inst->getAddr()];
    this_metrics.addr = inst->getAddr();
    this_metrics.instructions++;
    if(ptaken == taken) { 
        this_metrics.overall++; 
    }
    if(globalTaken == taken) { this_metrics.global++; }
    if(localTaken == taken) { this_metrics.local++; }
    metrics[inst->getAddr()] = this_metrics; 

    //printf("Added metrics");
    if (taken != ptaken) {
        if (doUpdate)
            btb.updateOnly(inst,oracleID);
        return MissPrediction;
    }
    //if(globalTaken != localTaken)
    //  printf("%X %X,%s,%s,%s\n", inst->getAddr(), 16, ptaken?"True":"False", globalTaken?"True":"False", localTaken?"True":"False");
    /*metrics_t this_metrics = metrics[inst->getAddr()];
    this_metrics.instructions++;
    if(ptaken == taken) { 
        this_metrics.overall++; 
