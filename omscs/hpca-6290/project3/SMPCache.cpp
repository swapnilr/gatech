...
std::map<PAddr, int32_t> SMPCache::lastWriter;
std::map<PAddr, int32_t> SMPCache::newBlockCnt;
std::map<PAddr, int32_t> SMPCache::ownBlockCnt;
std::map<PAddr, int32_t> SMPCache::getBlockCnt;
...
SMPCache::~SMPCache()
{
//---------- Processor NEW block 0 ----------
//P(0)_DL1 Top - 4981552:263176
//P(0)_DL1 2nd - 4985060:262145
//---------- Processor OWN block 0 ----------
//P(0)_DL1 Top - 4986868:263881
//P(0)_DL1 2nd - 4986848:263369
//---------- Processor GET block 0 ----------
//P(0)_DL1 Top - 4986860:1770
//P(0)_DL1 2nd - 4986876:1770
    if(getNodeID() == 0) {
      PAddr paddr;
      paddr = 4981552;
      printf("%d: New - %d, Own - %d, Get - %d\n", paddr, SMPCache::newBlockCnt[paddr], SMPCache::ownBlockCnt[paddr], SMPCache::getBlockCnt[paddr]); 
      paddr = 4985060;
      printf("%d: New - %d, Own - %d, Get - %d\n", paddr, SMPCache::newBlockCnt[paddr], SMPCache::ownBlockCnt[paddr], SMPCache::getBlockCnt[paddr]); 
      paddr = 4986868;
      printf("%d: New - %d, Own - %d, Get - %d\n", paddr, SMPCache::newBlockCnt[paddr], SMPCache::ownBlockCnt[paddr], SMPCache::getBlockCnt[paddr]); 
    ...
    // Convert to getNodeID() == 0 to print the top 2 instructions in each
    if(getNodeID() == -1) {
      PAddr top;
      int32_t top_count = 0;
      PAddr second;
      int32_t second_count = 0;
      for(std::map<PAddr, int32_t>::iterator iter=SMPCache::newBlockCnt.begin(); iter != SMPCache::newBlockCnt.end(); iter++) {
         if(iter->second > top_count) {
           second = top;
           second_count = top_count;
           top = iter->first;
           top_count = iter->second;
         } else if(iter->second > second_count) {
           second = iter->first;
           second_count = iter->second;
         }
         //printf("%s-%d:%d\n", getSymbolicName(), iter->first, iter->second);
      }
      if (top_count > 0) {
        printf("---------- Processor NEW block %d ----------\n", getNodeID());
        printf("%s Top - %d:%d\n", getSymbolicName(), top, top_count);
        printf("%s 2nd - %d:%d\n", getSymbolicName(), second, second_count);
      }
      top_count = 0;
      second_count=0;

      for(std::map<PAddr, int32_t>::iterator iter=SMPCache::ownBlockCnt.begin(); iter != SMPCache::ownBlockCnt.end(); iter++) {
         if(iter->second > top_count) {
           second = top;
           second_count = top_count;
           top = iter->first;
      ...
      for(std::map<PAddr, int32_t>::iterator iter=SMPCache::getBlockCnt.begin(); iter != SMPCache::getBlockCnt.end(); iter++) {
         if(iter->second > top_count) {
           second = top;
           second_count = top_count;
           top = iter->first;
...
{
    //printf("%s, %d\n", getSymbolicName(), mreq->isDataReq());
    PAddr addr;
...
void SMPCache::read(MemRequest *mreq)
{
     if(mreq->isDataReq()) {
      ...
      //printf("%d, %d\n", mreq->getGProcessor()->getId(), addr);
      if(SMPCache::lastWriter.count(paddr) == 0) {
        if(SMPCache::newBlockCnt.count(paddr) == 0) {
        ...
        if(SMPCache::getBlockCnt.count(paddr) == 0) {
          SMPCache::getBlockCnt[paddr] = 1;
        } else {
          SMPCache::getBlockCnt[paddr] += 1;
        }
      }
